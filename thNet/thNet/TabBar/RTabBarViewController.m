...
@interface RTabBarViewController ()<UINavigationControllerDelegate,RTabBarDelegate>
...
- (void)viewDidLoad {
    ...
    self.tabBar = [[RTabBarView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - self.tabBarHeight, CGRectGetWidth(self.view.bounds), self.tabBarHeight)];
...
-(void) loadViewControllers {
    ...
    for (UIViewController *controller in self.viewControllers) {
        ...
        RTabBarItemView *tabItem = [[[NSBundle mainBundle] loadNibNamed:@"RTabBarItemView" owner:nil options:nil] objectAtIndex:0];
...
- (void)handleFriendTimelineUreadEvent {
    ...
    RTabBarItemView* tabBarItemView = nil;
...
- (void)tabBar:(RTabBarView *)aTabBar didSelectTabAtIndex:(NSUInteger)anIndex{
    ...
    if (self.selectedViewController == vc) {
        ...
        if (!aTabBar.simulateSelected) {
            id<RTabBarControllerSubVcProtocol> protocol = (id<RTabBarControllerSubVcProtocol>)self.selectedViewController;
...
@implementation UIViewController (RTabBarControllerItem)

//@dynamic tabController;

static const char* RTabControllerKey = "RTabControllerKey";

-(RTabBarViewController *)tabController{
    return objc_getAssociatedObject(self, RTabControllerKey);
}
-(void)setTabController:(RTabBarViewController *)tabController{
    objc_setAssociatedObject(self, RTabControllerKey, tabController, OBJC_ASSOCIATION_ASSIGN);
}
